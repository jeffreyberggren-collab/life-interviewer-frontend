// Robust init + logs. If an element or config is missing, show a banner.
(function(){
  function el(id){ return document.getElementById(id); }
  const banner = el('banner');
  function showBanner(msg){ if(banner){ banner.textContent=msg; banner.classList.remove('hidden'); } alert(msg); }

  const logEl = el('log');
  function log(msg){ console.log(msg); if(logEl){ logEl.textContent = ((logEl.textContent + '\n' + msg).trim()); } }

  function req(id){ const n=el(id); if(!n){ showBanner(`Missing element #${id}. Did the HTML upload correctly?`); throw new Error('Missing '+id); } return n; }

  window.addEventListener('DOMContentLoaded', () => {
    try { main(); } catch (e){ console.error(e); showBanner(e.message || 'Init failed'); }
  });

  async function main(){
    const cfg = (window.APP_CONFIG||{});
    const SERVER_URL = cfg.SERVER_URL;
    if(!SERVER_URL){ showBanner('SERVER_URL is empty. Open config.js and set your Render URL.'); throw new Error('SERVER_URL missing'); }

    const preview = req('preview');
    const overlayCanvas = req('overlayCanvas');
    const qBanner = req('questionBanner');
    const downloads = req('downloads');
    const progressBar = req('progressBar');
    const assistantAudio = req('assistantAudio');

    const eventSelect = req('eventSelect');
    const langSelect = req('langSelect');
    const overlayToggle = req('overlayToggle');
    const captionSize = req('captionSize');
    const captionSizeVal = req('captionSizeVal');
    const connectBtn = req('connectBtn');
    const startRecBtn = req('startRecBtn');
    const stopRecBtn = req('stopRecBtn');
    const srtBtn = req('srtBtn');
    ...
    connectBtn.addEventListener('click', async ()=>{
      try{ await connectRealtime(); }catch(e){ showBanner('Connect failed: '+(e.message||e)); }
    });
    startRecBtn.addEventListener('click', async ()=>{
      try{ await startRecording(); }catch(e){ showBanner('Start recording failed: '+(e.message||e)); }
    });
    stopRecBtn.addEventListener('click', ()=>{ try{ stopRecording(); }catch(e){ showBanner('Stop failed: '+(e.message||e)); } });
    srtBtn.addEventListener('click', ()=>{ try{ exportSrt(); }catch(e){ showBanner('SRT export failed: '+(e.message||e)); } });
    ...
    async function connectRealtime(){
      ...
      const u = new URL('/session', SERVER_URL);
      u.searchParams.set('event', eventSelect.value);
      u.searchParams.set('vibe', 'old_friend');
      const resp = await fetch(u.toString());
      if(!resp.ok){ throw new Error('Server /session failed: '+resp.status); }
      const session = await resp.json();
      const EPHEMERAL_KEY = session?.client_secret?.value;
      if(!EPHEMERAL_KEY){ throw new Error('No session token received'); }

      pc = new RTCPeerConnection();
      // mic up to model
      localStream.getAudioTracks().forEach(t=>pc.addTrack(t, localStream));
      // assistant audio back
      ...
      const apiUrl = 'https://api.openai.com/v1/realtime?model=gpt-4o-realtime-preview';
      const sdpResp = await fetch(apiUrl, {
        method: 'POST',
        body: offer.sdp,
    ...
    function appendCaptionDelta(delta){
      if(currentCaption.i===null){ currentCaption = { i: captionIndex++, start: nowMs(), text: '' }; }
    ...
    function exportSrt(){
      if(!captions.length){ showBanner('No captions yet.'); return; }
    ...
    async function startRecording(){
      ...
      if (localStream?.getAudioTracks()?.length){
        audioCtx.createMediaStreamSource(new MediaStream([localStream.getAudioTracks()[0]])).connect(mixed);
      }
      if (remoteStream?.getAudioTracks()?.length){
        audioCtx.createMediaStreamSource(new MediaStream([remoteStream.getAudioTracks()[0]])).connect(mixed);
      }
    ...
    function stopRecording(){
      if(!mediaRecorder){ showBanner('Recorder not started'); return; }
      mediaRecorder.stop();
